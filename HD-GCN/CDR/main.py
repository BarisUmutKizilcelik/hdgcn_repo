import os
import random
from hdgcn.feeder_ntu import Feeder
from hdgcn.HDGCN import Model
from hdgcn.ntu_rgb_d_hierarchy import Graph
import torch
...
parser.add_argument('--result_dir', type=str, help='dir to save result txt files', default='/cvhci/temp/prgc/hdgcn_filtered/CDR/output_0/results_cdr/')
...
parser.add_argument('--model_type', type=str, help='[ce, ours]', default='hdgcn')
...
def load_data(args):
    ...
    if args.dataset=='ntu60':
        ...
        args.num_gradual = 20
        args.train_len = int(50000 * 0.9)
        train_dataset = data_load.ntu_dataset(True,
                                        transform = transforms.Compose([
                                        transforms.RandomCrop(32, padding=4),
                                        transforms.RandomHorizontalFlip(),
                                        transforms.ToTensor(),
        ...
        val_dataset = data_load.ntu_dataset(False,
                                        transform = transforms.Compose([
                                        transforms.ToTensor(),
                                        transforms.Normalize((0.4914, 0.4822, 0.4465),(0.2023, 0.1994, 0.2010)),
                                        ]),
        ...
        test_dataset = data_load.ntu_test_dataset(
                                        transform = transforms.Compose([
                                        transforms.ToTensor(),
                                        transforms.Normalize((0.4914, 0.4822, 0.4465),(0.2023, 0.1994, 0.2010)),
                                        ]),
...
def train_one_step(net, data, label, optimizer, criterion, nonzero_ratio, clip):
    ...
    optimizer.step()
    optimizer.zero_grad()
    acc = accuracy(pred['logits'], label, topk=(1,))
...
def train(train_loader, epoch, model1, optimizer1, args):
    ...
    for i, (data, labels, indexes) in enumerate(train_loader):
        ...
        prec1,  = accuracy(logits1['logits'], labels, topk=(1, ))
        train_total+=1
        train_correct+=prec1
        # Loss transfer 
...
def evaluate(test_loader, model1):
    ...
    with torch.no_grad():
        for data, labels, _ in test_loader:
            data = data.cuda()
            logits1 = model1(data)
            outputs1 = F.softmax(logits1['logits'], dim=1)
...
def main(args):
    ...
    model_str = args.dataset + '_%s_' % args.model_type + args.noise_type + '_' + str(0) + '_' + str(args.seed)
    ...
    train_loader = torch.utils.data.DataLoader(
                dataset=Feeder(data_path='/cvhci/temp/prgc/hdgcn_filtered/origin/no_noise/noisy_dataset/NTU60_CV.npz', split= 'train', debug= False, random_choose= False, random_shift= False, random_move=False, window_size=16, normalization= False, random_rot=True, p_interval=[0.5, 1], bone= False),
    ...
    test_loader = torch.utils.data.DataLoader(
            dataset=Feeder(data_path='/cvhci/temp/prgc/hdgcn_filtered/origin/no_noise/noisy_dataset/NTU60_CV.npz', split= 'test', window_size=16, p_interval=[0.95], bone= False, debug=False),
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
            drop_last=False,
    ...
    if args.dataset == 'mnist':
        ...
        clf1 = Model(graph=Graph(labeling_mode= 'spatial',CoM=1),compute_flops= True)
        optimizer1 = torch.optim.SGD(clf1.parameters(), lr=learning_rate, weight_decay=args.weight_decay, momentum=0.9)
        scheduler1 = MultiStepLR(optimizer1, milestones=[10, 30], gamma=0.1)
    ...
    epoch + 1, args.n_epoch, 31, test_acc1))
    # save results
    with open(txtfile, "a") as myfile:
    ...
    torch.save(clf1.state_dict(), args.result_dir + '/' + 'model.pt')
    return test_acc_max
