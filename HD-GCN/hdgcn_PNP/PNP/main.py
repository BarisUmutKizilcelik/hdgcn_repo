...
def build_logger(params):
    logger_root = f'/cvhci/temp/prgc/hdgcn_filtered/Results/{params.dataset}'
...
def build_model_optim_scheduler(params, device, build_scheduler=True):
    ...
    net = Model(graph=Graph(labeling_mode= 'spatial',CoM=1),compute_flops= True)  # C+1 classfier #model added
    if params.opt == 'sgd':
...
def build_dataset_loader(params):
    ...
    # data_path="/cvhci/temp/prgc/hdgcn_filtered/filtered_gen_data/xview/train_data.npy"
    # label_path="/cvhci/temp/prgc/hdgcn_filtered/filtered_gen_data/xview/train_label.pkl"
    # test_data_path="/cvhci/temp/prgc/hdgcn_filtered/filtered_gen_data/xview/val_data.npy"
    # test_label_path="/cvhci/temp/prgc/hdgcn_filtered/filtered_gen_data/xview/val_label.pkl"
    data_path="/cvhci/temp/prgc/st-gcn/data/noise_20/xview/train_data.npy"
    label_path="/cvhci/temp/prgc/st-gcn/data/noise_20/xview/train_label.pkl"
    test_data_path="/cvhci/temp/prgc/st-gcn/data/noise_20/xview/test_data.npy"
    test_label_path="/cvhci/temp/prgc/st-gcn/data/noise_20/xview/test_label.pkl"
    
    train_loader = torch.utils.data.DataLoader(
                dataset=Feeder_triple(data_path, label_path, shear_amplitude=0.3, temperal_padding_ratio=0, aug_method='1'),
    ...
    test_loader = torch.utils.data.DataLoader(
            dataset=Feeder_triple(test_data_path, test_label_path, shear_amplitude=0.0),
            batch_size=params.batch_size,
            shuffle=False,
            num_workers=3,
            drop_last=False,
...
def main(cfg, device):
    ...
    train_loader, test_loader = build_dataset_loader(cfg)

    logger.msg(f"Categories: {n_classes}")
    ...
    for epoch in range(0, cfg.epochs):
        ...
        for it, (data, label) in enumerate(pbar):
