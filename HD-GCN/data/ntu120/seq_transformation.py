...
root_path = '/cvhci/temp/prgc/hdgcn_filtered/origin/no_noise'
stat_path = osp.join(root_path, 'statistics')
setup_file = osp.join(stat_path, 'accsetup.txt')
camera_file = osp.join(stat_path, 'acccamera.txt')
performer_file = osp.join(stat_path, 'accperformer.txt')
replication_file = osp.join(stat_path, 'accreplication.txt')
label_file = osp.join(stat_path, 'acclabel.txt')
skes_name_file = osp.join(stat_path, 'filtered_skes_avaliable_name.txt')

denoised_path = osp.join(root_path, 'denoised_data')
raw_skes_joints_pkl = osp.join(denoised_path, 'raw_denoised_joints.pkl')
frames_file = osp.join(denoised_path, 'frames_cnt.txt')

save_path = '/cvhci/temp/prgc/hdgcn_filtered/origin/no_noise/noisy_dataset'
...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path):
    ...
    save_name = '/cvhci/temp/prgc/hdgcn_filtered/origin/no_noise/noisy_dataset/NTU60_%s.npz' % evaluation
